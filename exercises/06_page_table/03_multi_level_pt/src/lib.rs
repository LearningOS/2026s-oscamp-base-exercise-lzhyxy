...
impl Sv39PageTable {
    ...
    let level1_ppn = {
...

        {
            ...
            pte2_val = root.entries[vpn2];
        }

        (pte2_val >> PPN_SHIFT) & 0xFFFF_FFFF
    };
    ...
    let level0_ppn = {
        ...
        {
            let node = self.nodes.get_mut(&level1_ppn).unwrap();
            let pte1 = node.entries[vpn1];

            if (pte1 & PTE_V) == 0 {
                let new_ppn = self.alloc_node();

                let node = self.nodes.get_mut(&level1_ppn).unwrap();
                node.entries[vpn1] = (new_ppn << PPN_SHIFT) | PTE_V;
            }

            let node = self.nodes.get(&level1_ppn).unwrap();
            pte1_val = node.entries[vpn1];
        }

        (pte1_val >> PPN_SHIFT) & 0xFFFF_FFFF
    };

    // ===== level 0 =====
    {
        let node = self.nodes.get_mut(&level0_ppn).unwrap();
        let pa_ppn = pa >> 12;
        node.entries[vpn0] = (pa_ppn << PPN_SHIFT) | flags | PTE_V;
    }
    }
    ...
    let pte2 = node.entries[vpn2];
    if (pte2 & PTE_V) == 0 {
        return TranslateResult::PageFault;
    }

    if (pte2 & (PTE_R | PTE_W | PTE_X)) != 0 {
        let ppn = (pte2 >> PPN_SHIFT) & 0xFFFF_FFFF;
        let offset = va & 0x3FFFFF; // 2MB offset (22 bits incl alignment safety)
        return TranslateResult::Ok((ppn << 12) | offset);
    }

    current_ppn = (pte2 >> PPN_SHIFT) & 0xFFFF_FFFF;
    ...
    if (pte1 & (PTE_R | PTE_W | PTE_X)) != 0 {
        let ppn = (pte1 >> PPN_SHIFT) & 0xFFFF_FFFF;
        let offset = va & 0x1F_FFFF; // ✅ 2MB offset
        return TranslateResult::Ok((ppn << 12) | offset);
    }

    current_ppn = (pte1 >> PPN_SHIFT) & 0xFFFF_FFFF;
    ...
    let ppn = (pte0 >> PPN_SHIFT) & 0xFFFF_FFFF;
    let offset = va & 0xFFF;

    TranslateResult::Ok((ppn << 12) | offset)
    }
    ...
    let level1_ppn = {
        ...
        let pte2 = {
            let root = self.nodes.get_mut(&root_ppn).unwrap();
            root.entries[vpn2]
        };

        let ppn = if (pte2 & PTE_V) == 0 {
            let new_ppn = self.alloc_node();
    ...
    // ===== level 1 (leaf) =====
    {
        let node = self.nodes.get_mut(&level1_ppn).unwrap();
        let pa_ppn = pa >> 12;
        node.entries[vpn1] = (pa_ppn << PPN_SHIFT) | flags | PTE_V;
    }
}
    }
